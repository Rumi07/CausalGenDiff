...
def split_dataset_with_gene_names(dataset, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1, 
                                  random_state=None, json_file=None):

    if json_file is None:
        raise ValueError("A valid JSON file path must be provided.")
    
    print(f"JSON file path: {json_file}")

    # Check if the JSON file with indices exists
    if os.path.exists(json_file):
        print(f"Loading dataset indices from {json_file}...")
        ...
        print("Splitting dataset and saving indices to JSON...")
    ...
    print(f"Saved dataset indices to {json_file}")
