...
class TimestepEmbedder(nn.Module):
    ...
    Convert time emb into frequency_embedding_size dimension, and then project it to hidden_size
    """
...
class DiT_diff(nn.Module):
    def __init__(self,
        ...
        self.in_layer = nn.Sequential(
            nn.Linear(st_input_size, hidden_size), #[in_features=4784, out_features=256, bias=true]
            # nn.Dropout(p=0.5)
        )
        self.x_in_layer = nn.Sequential(
            nn.Linear(condi_input_size, hidden_size) #[in_featu=6143, out_features=256]
        )
        self.cond_layer = nn.Sequential(
            nn.Linear(self.condi_input_size, hidden_size), #[in_featu=6143, out_features=256]
            # nn.Dropout(p=0.5)
        )
